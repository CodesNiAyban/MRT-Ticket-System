	...
	try {
		...
		if (newMaintenance === false) {
			...
			if (hasDisconnectedStations) {
				...
				return res.status(400).json({message: "Unable to deploy MRT Online Tap because there are disconnected stations."});
			}
			...
			if (activeBeepCards.length > 0) {
				...
				return res.status(400).json({message: "Cannot activate maintenance while there are active beep cards."});
			}
		}
